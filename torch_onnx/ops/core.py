...
from torch_onnx.ops import common as common_ops
...
from torch_onnx._builder import OpBuilder
from onnxscript import ir
import torch
from torch_onnx._torch_tensor import TorchTensor
...
is_scalar = common_ops.is_scalar

def constant(op: OpBuilder, value, dtype: torch.dtype | None = None):
    tensor = TorchTensor(torch.tensor(value, dtype))
    return op.Constant(value=tensor)
...
def is_constant(value: ir.Value) -> bool:
    return value.const_value is not None


@torch_op("aten::_local_scalar_dense")
def aten__local_scalar_dense(op: OpBuilder, self: ir.Value[Union[FLOAT16, FLOAT, DOUBLE, BFLOAT16, IntType]]):
    """_local_scalar_dense(Tensor self) -> Scalar"""

    # Return the first element in tensor as a scalar.
    if is_int_value(self):
        if is_constant(self):
            return constant(op, torch.from_dlpack(self.const_value).data[0], torch.int64)
        else:
            return op.Cast(op.Gather(op.Reshape(self, [-1]), 0), to=ir.DataType.INT64)
    else:
        if is_constant(self):
            return constant(op, torch.from_dlpack(self.const_value).data[0], torch.float32)
        else:
            return op.Cast(op.Gather(op.Reshape(self, [-1]), 0), to=ir.DataType.FLOAT)


@torch_op("aten::_log_softmax", trace_only=True)
def aten__log_softmax_half(op,
    ...
    return aten__log_softmax(op, self, dim, half_to_float)


@torch_op("aten::_log_softmax", traceable=True)
def aten__log_softmax(op,
...
def aten__softmax_half(op: OpBuilder, self: Union[FLOAT16, BFLOAT16], dim: int, half_to_float: bool) -> FLOAT:
    ...
    return aten_softmax_no_dtype(op, self, dim)


@torch_op("aten::_softmax", trace_only=True)
def aten__softmax(op,
...
def aten_abs(op: OpBuilder, self: TRealOrUInt8) -> TRealOrUInt8:
...
def aten_abs_complex(op: OpBuilder, self: TRealOrUInt8) -> TRealOrUInt8:
    """abs(Tensor self) -> Tensor"""
    # self_real = self[..., 0]
...
@torch_op("aten::acos")
def aten_acos(op: OpBuilder, self: TFloat) -> TFloat:
...
@torch_op("aten::acosh")
def aten_acosh(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_add(op: OpBuilder, self: TReal, other: TReal, alpha: float = 1.0) -> TReal:
...
def aten_add_complex(op: OpBuilder, self: TReal, other: TReal, alpha: float = 1.0) -> TReal:
    """add.Tensor(Tensor self, Tensor other, *, Scalar alpha=1) -> Tensor"""

    return aten_add(op, self, other, alpha=alpha)
...
def aten_addcdiv(op: OpBuilder, self: TFloat, tensor1: TFloat, tensor2: TFloat, value: float = 1.0) -> TFloat:
...
def aten_adjoint(op: OpBuilder, self: TensorType) -> TensorType:
    """adjoint(Tensor(a) self) -> Tensor(a)"""

    raise NotImplementedError()


def aten_affine_grid_generator(op,
...
def aten_affine_grid_generator_backward(op,
...
@torch_op("aten::alias")
def aten_alias(op: OpBuilder, self: TTensor) -> TTensor:
    """alias(Tensor(a) self) -> Tensor(a)"""

    return op.Identity(self)


def aten_alias_copy(op: OpBuilder, self: TensorType) -> TensorType:
    """alias_copy(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_align_as(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """align_as(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_align_tensors(op: OpBuilder, tensors: Sequence[TensorType]) -> TensorType:
    """align_tensors(Tensor[] tensors) -> Tensor[]"""

    raise NotImplementedError()


def aten_align_to(op: OpBuilder, self: TensorType, names: Sequence[str]) -> TensorType:
...
def aten_all(op: OpBuilder, self: TTensor) -> BOOL:
    ...
    if is_scalar(self):
        ...
        self_int = op.Cast(self_bool, to=INT64)
...
def aten_all_dim(op: OpBuilder, self: TTensor, dim: int, keepdim: bool = False) -> BOOL:
...
def aten_all_dims(op: OpBuilder, self: TTensor, dim: Sequence[int] = (), keepdim: bool = False) -> BOOL:
    """all.dims(Tensor self, int[]? dim=None, bool keepdim=False) -> Tensor"""

    if not dim:
        return aten_all_dims_no_dim(op, self, keepdim)
    for d in dim:
...
def aten_all_dims_no_dim(op: OpBuilder, self: TTensor, keepdims: bool) -> BOOL:
    """all.dims(Tensor self, int[]? dim=None, bool keepdim=False) -> Tensor"""
...
def aten_alpha_dropout(op: OpBuilder, input: TensorType, p: float, train: bool) -> TensorType:
...
@torch_op("aten::amax")
def aten_amax(op: OpBuilder, self: TRealOrUInt8, dim: INT64, keepdim: bool = False) -> TRealOrUInt8:
...
@torch_op("aten::amin")
def aten_amin(op: OpBuilder, self: TRealOrUInt8, dim: INT64, keepdim: bool = False) -> TRealOrUInt8:
...
def aten_and(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """__and__.Tensor(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_angle(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_any(op: OpBuilder, self: TTensor) -> BOOL:
...
def aten_any_dim(op: OpBuilder, self: TTensor, dim: int, keepdim: bool = False) -> BOOL:
...
def aten_any_dims(op: OpBuilder, self: TTensor, dim: Sequence[int] = (), keepdim: bool = False) -> BOOL:
    """any.dims(Tensor self, int[1]? dim=None, bool keepdim=False) -> Tensor"""

    if not dim:
        return aten_any_dims_no_dim(op, self, keepdim)
    for d in dim:
...
def aten_any_dims_no_dim(op: OpBuilder, self: TTensor, keepdims: bool) -> BOOL:
    """any.dims(Tensor self, int[1]? dim=None, bool keepdim=False) -> Tensor"""
...
def aten_arange(op: OpBuilder, end: Union[DOUBLE, FLOAT, INT16, INT32, INT64], dtype: int = -1) -> TensorType:
...
def aten_arccos(op: OpBuilder, self: TensorType) -> TensorType:
    """arccos(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_arccosh(op: OpBuilder, self: TensorType) -> TensorType:
    """arccosh(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_arcsin(op: OpBuilder, self: TensorType) -> TensorType:
    """arcsin(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_arcsinh(op: OpBuilder, self: TensorType) -> TensorType:
    """arcsinh(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_arctan(op: OpBuilder, self: TensorType) -> TensorType:
    """arctan(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_arctan2(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """arctan2(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_arctanh(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_argmax(op: OpBuilder, self: Union[RealType, UINT8], keepdim: bool = False) -> INT64:
    """argmax(Tensor self, int? dim=None, bool keepdim=False) -> Tensor"""

    self_is_scaler = is_scalar(self)
...
@torch_op("aten::argmax", traceable=True)
def aten_argmax_dim(op: OpBuilder, self: Union[RealType, UINT8], dim: int, keepdim: bool = False) -> INT64:
    """argmax(Tensor self, int? dim=None, bool keepdim=False) -> Tensor"""
...
def aten_argmin(op: OpBuilder, self: Union[RealType, UINT8], keepdim: bool = False) -> INT64:
...
@torch_op("aten::argmin", traceable=True)
def aten_argmin_dim(op: OpBuilder, self: Union[RealType, UINT8], dim: int, keepdim: bool = False) -> INT64:
    """argmin(Tensor self, int? dim=None, bool keepdim=False) -> Tensor"""
...
def aten_argsort(op: OpBuilder, self: TensorType, dim: int = -1, descending: bool = False) -> TensorType:
    """argsort(Tensor self, int dim=-1, bool descending=False) -> Tensor"""

    raise NotImplementedError()


def aten_argwhere(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_as_strided_scatter(op,
    self: TensorType,
    src: TensorType,
    size: INT64,
    stride: INT64,
...
@torch_op("aten::asin")
def aten_asin(op: OpBuilder, self: TFloat) -> TFloat:
...
@torch_op("aten::asinh")
def aten_asinh(op: OpBuilder, self: TFloat) -> TFloat:
...
@torch_op("aten::atan")
def aten_atan(op: OpBuilder, self: TFloat) -> TFloat:
...
@torch_op("aten::atan2")
def aten_atan2(op: OpBuilder, self: TFloat, other: TFloat) -> TFloat:
...
@torch_op("aten::atanh")
def aten_atanh(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_atleast_1d(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_atleast_1d_sequence(op: OpBuilder, self: Sequence[TTensor]) -> TTensor:
...
def aten_atleast_2d(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_atleast_2d_sequence(op: OpBuilder, self: Sequence[TTensor]) -> TTensor:
...
def aten_atleast_3d(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_atleast_3d_sequence(op: OpBuilder, self: Sequence[TTensor]) -> TTensor:
...
def aten_bartlett_window(op: OpBuilder, window_length: int) -> TensorType:
...
def aten_batch_norm_backward_elemt(op,
    grad_out: TensorType,
    input: TensorType,
    mean: TensorType,
    invstd: TensorType,
...
def aten_batch_norm_backward_reduce(op,
    grad_out: TensorType,
    input: TensorType,
    mean: TensorType,
    invstd: TensorType,
...
def aten_batch_norm_gather_stats(op,
    input: TensorType,
    mean: TensorType,
    invstd: TensorType,
    running_mean: Optional[TensorType],
...
def aten_batch_norm_gather_stats_with_counts(op,
    input: TensorType,
    mean: TensorType,
    invstd: TensorType,
    running_mean: Optional[TensorType],
...
def aten_batch_norm_stats(op: OpBuilder, input: TensorType, eps: float) -> tuple[TensorType, TensorType]:
    """batch_norm_stats(Tensor input, float eps) -> (Tensor, Tensor)"""

    raise NotImplementedError()


def aten_batch_norm_update_stats(op,
    input: TensorType,
    running_mean: Optional[TensorType],
    running_var: Optional[TensorType],
    momentum: float,
...
def aten_bernoulli(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_bernoulli_p(op: OpBuilder, self: TTensor, p: float) -> TTensor:
...
def aten_binary_cross_entropy_with_logits(op,
    self: TensorType,
    target: TensorType,
...
def aten_bitwise_and(op: OpBuilder, self: TInt, other: TInt) -> TInt:
...
def aten_bitwise_left_shift_int16(op: OpBuilder, self: INT16, other: INT16) -> INT16:
...
def aten_bitwise_left_shift_int64(op: OpBuilder, self: INT64, other: INT64) -> INT64:
    ...
    return op.Cast(result, to=INT64)


@torch_op("aten::bitwise_left_shift")
def aten_bitwise_left_shift_int8(op: OpBuilder, self: INT8, other: INT8) -> INT8:
    """bitwise_left_shift.Tensor(Tensor self, Tensor other) -> Tensor"""
    # assert other >= 0
...
def aten_bitwise_not(op: OpBuilder, self: TInt) -> TInt:
...
def aten_bitwise_or(op: OpBuilder, self: TInt, other: TInt) -> TInt:
...
def aten_bitwise_right_shift_int16(op: OpBuilder, self: INT16, other: INT16) -> INT16:
...
def aten_bitwise_right_shift_int64(op: OpBuilder, self: INT64, other: INT64) -> INT64:
    ...
    # Choose the shifted value based on the sign bit
    return op.Where(
        negative, op.Cast(negative_shifted, to=INT64), op.Cast(shifted, to=INT64)
    )


@torch_op("aten::bitwise_right_shift")
def aten_bitwise_right_shift_int8(op: OpBuilder, self: INT8, other: INT8) -> INT8:
    """bitwise_right_shift.Tensor(Tensor self, Tensor other) -> Tensor"""
    negative = op.Less(self, 0)
    self = op.Cast(self, to=UINT8.dtype)
    other = op.Cast(other, to=UINT8.dtype)
...
def aten_bitwise_xor(op: OpBuilder, self: TInt, other: TInt) -> TInt:
...
def aten_blackman_window(op: OpBuilder, window_length: int) -> TensorType:
...
def aten_block_diag(op: OpBuilder, tensors: Sequence[TensorType]) -> TensorType:
...
@torch_op("aten::bmm")
def aten_bmm(op: OpBuilder, self: TFloat, mat2: TFloat) -> TFloat:
    """bmm(Tensor self, Tensor mat2) -> Tensor"""

    return op.MatMul(self, mat2)


def aten_broadcast_tensors(op: OpBuilder, tensors: Sequence[TensorType]) -> TensorType:
...
def aten_broadcast_to(op: OpBuilder, self: TTensor, size: INT64) -> TTensor:
    """broadcast_to(Tensor(a) self, SymInt[] size) -> Tensor(a)"""

    return op.Expand(self, size)


def aten_bucketize(op,
...
def aten_can_cast(op: OpBuilder, from_: int, to: int) -> bool:
    """can_cast(ScalarType from, ScalarType to) -> bool"""

    raise NotImplementedError()


def aten_cartesian_prod(op: OpBuilder, tensors: Sequence[TensorType]) -> TensorType:
...
def aten_cat_complex(op: OpBuilder, tensors: Sequence[TTensor], dim: int = 0) -> TTensor:
...
def aten_ccol_indices_copy(op: OpBuilder, self: TensorType) -> TensorType:
...
@torch_op("aten::ceil")
def aten_ceil(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_chain_matmul(op: OpBuilder, matrices: Sequence[TensorType]) -> TensorType:
    """chain_matmul(Tensor[] matrices) -> Tensor"""

    raise NotImplementedError()


def aten_chalf(op: OpBuilder, self: TensorType, memory_format: Optional[str] = None) -> TensorType:
    """chalf(Tensor self, *, MemoryFormat? memory_format=None) -> Tensor"""

    raise NotImplementedError()


def aten_channel_shuffle(op: OpBuilder, self: TensorType, groups: int) -> TensorType:
    """channel_shuffle(Tensor self, int groups) -> Tensor"""

    raise NotImplementedError()


def aten_cholesky(op: OpBuilder, self: TensorType, upper: bool = False) -> TensorType:
    """cholesky(Tensor self, bool upper=False) -> Tensor"""

    raise NotImplementedError()


def aten_cholesky_inverse(op: OpBuilder, self: TensorType, upper: bool = False) -> TensorType:
    """cholesky_inverse(Tensor self, bool upper=False) -> Tensor"""

    raise NotImplementedError()


def aten_cholesky_solve(op,
...
def aten_choose_qparams_optimized(op,
...
@torch_op("aten::chunk")
def aten_chunk(op: OpBuilder, self: TTensor, chunks: int, dim: int = 0) -> Sequence[TTensor]:
    ...
    num_per_chunk = op.Cast(op.Mod(dim_size, chunks) > 0, to=INT64) + num_per_chunk  # type: ignore[operator]
...
def aten_clamp(op: OpBuilder, self: TReal, min: Optional[TReal] = None, max: Optional[TReal] = None) -> TReal:
...
def aten_clamp_max(op: OpBuilder, self: TReal, max_: TReal) -> TReal:
...
def aten_clamp_min(op: OpBuilder, self: TReal, min_: TReal) -> TReal:
...
def aten_coalesce(op: OpBuilder, self: TensorType) -> TensorType:
    """coalesce(Tensor(a) self) -> Tensor(a)"""

    raise NotImplementedError()


def aten_col_indices(op: OpBuilder, self: TensorType) -> TensorType:
    """col_indices(Tensor(a) self) -> Tensor(a)"""

    raise NotImplementedError()


def aten_col_indices_copy(op: OpBuilder, self: TensorType) -> TensorType:
    """col_indices_copy(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_column_stack(op: OpBuilder, tensors: Sequence[TensorType]) -> TensorType:
    """column_stack(Tensor[] tensors) -> Tensor"""

    raise NotImplementedError()


def aten_combinations(op,
...
def aten_complex(op: OpBuilder, real: TFloat, imag: TFloat) -> TFloat:
...
def aten_concat(op: OpBuilder, tensors: Sequence[TTensor], dim: int = 0) -> TTensor:
...
def aten_concatenate(op: OpBuilder, tensors: Sequence[TTensor], dim: int = 0) -> TTensor:
...
@torch_op("aten::conj")
def aten_conj(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_conj_complex(op: OpBuilder, self: TFloat) -> TFloat:
    """conj(Tensor(a) self) -> Tensor(a)"""
...
def aten_conj_physical(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_constant_pad_nd(op: OpBuilder, self: TTensor, pad: INT64, value: float = 0.0) -> TTensor:
...
def aten_convolution_backward(op,
...
def aten_convolution_backward_overrideable(op,
    grad_output: TensorType,
    input: TensorType,
    weight: TensorType,
    stride: Sequence[int],
...
def aten_convolution_overrideable(op,
    input: TensorType,
    weight: TensorType,
    bias: Optional[TensorType],
    stride: Sequence[int],
...
def aten_copysign(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """copysign.Tensor(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_corrcoef(op: OpBuilder, self: TensorType) -> TensorType:
...
@torch_op("aten::cos")
def aten_cos(op: OpBuilder, self: TFloat) -> TFloat:
...
@torch_op("aten::cosh")
def aten_cosh(op: OpBuilder, self: TFloat) -> TFloat:
    """cosh(Tensor self) -> Tensor"""

    return op.Cosh(self)


def aten_cosine_embedding_loss(op,
    input1: TensorType,
    input2: TensorType,
    target: TensorType,
    margin: float = 0.0,
...
def aten_count_nonzero(op: OpBuilder, self: TensorType, dim: Optional[int] = None) -> TensorType:
...
@torch_op("aten::cross")
def aten_cross(op: OpBuilder, self: TTensor, other: TTensor, dim: int = -1) -> TTensor:
...
def aten_crow_indices(op: OpBuilder, self: TensorType) -> TensorType:
    """crow_indices(Tensor(a) self) -> Tensor(a)"""

    raise NotImplementedError()


def aten_crow_indices_copy(op: OpBuilder, self: TensorType) -> TensorType:
    """crow_indices_copy(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_ctc_loss(op,
...
def aten_cudnn_affine_grid_generator(op,
...
def aten_cudnn_affine_grid_generator_backward(op,
...
def aten_cudnn_batch_norm_backward(op,
    input: TensorType,
    grad_output: TensorType,
    weight: TensorType,
    running_mean: Optional[TensorType],
...
def aten_cudnn_convolution_add_relu(op,
    self: TensorType,
    weight: TensorType,
    z: TensorType,
...
def aten_cudnn_convolution_relu(op,
    self: TensorType,
    weight: TensorType,
    bias: Optional[TensorType],
    stride: Sequence[int],
...
def aten_cudnn_convolution_transpose(op,
    self: TensorType,
    weight: TensorType,
    padding: Sequence[int],
    output_padding: Sequence[int],
...
def aten_cudnn_grid_sampler(op: OpBuilder, self: TensorType, grid: TensorType) -> TensorType:
    """cudnn_grid_sampler(Tensor self, Tensor grid) -> Tensor output"""

    raise NotImplementedError()


def aten_cudnn_grid_sampler_backward(op,
...
def aten_cudnn_is_acceptable(op: OpBuilder, self: TensorType) -> bool:
    """cudnn_is_acceptable(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_cummax(op: OpBuilder, self: TensorType, dim: int) -> tuple[TensorType, TensorType]:
    """cummax(Tensor self, int dim) -> (Tensor values, Tensor indices)"""

    raise NotImplementedError()


def aten_cummaxmin_backward(op,
...
def aten_cummin(op: OpBuilder, self: TensorType, dim: int) -> tuple[TensorType, TensorType]:
    """cummin(Tensor self, int dim) -> (Tensor values, Tensor indices)"""

    raise NotImplementedError()


def aten_cumprod(op: OpBuilder, self: TensorType, dim: int, dtype: Optional[int] = None) -> TensorType:
    """cumprod(Tensor self, int dim, *, ScalarType? dtype=None) -> Tensor"""

    raise NotImplementedError()


def aten_cumprod_backward(op,
...
def aten_data(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_deg2rad(op: OpBuilder, self: TFloat) -> TFloat:
    """deg2rad(Tensor self) -> Tensor"""

    return op.Mul(self, op.CastLike(_MATH_PI / 180.0, self))


def aten_dense_dim(op: OpBuilder, self: TensorType) -> int:
...
def aten_detach(op: OpBuilder, self: TensorType) -> TensorType:
    """detach(Tensor(a) self) -> Tensor(a)"""

    return op.Identity(self)


def aten_detach_copy(op: OpBuilder, self: TensorType) -> TensorType:
    """detach_copy(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_diag(op: OpBuilder, self: TensorType, diagonal: int = 0) -> TensorType:
    """diag(Tensor self, int diagonal=0) -> Tensor"""

    raise NotImplementedError()


def aten_diag_embed(op,
...
def aten_diagflat(op: OpBuilder, self: TensorType, offset: int = 0) -> TensorType:
...
def aten_diagonal(op: OpBuilder, self: TReal, offset: int = 0, dim1: int = 0, dim2: int = 1) -> TReal:
...
def aten_diagonal_bool(op: OpBuilder, self: BOOL, offset: int = 0, dim1: int = 0, dim2: int = 1) -> BOOL:
    """diagonal(Tensor(a) self, int offset=0, int dim1=0, int dim2=1) -> Tensor(a)"""
...
def _aten_diagonal_bool_onnx(
    ...
    tmp_tensor = op.ConstantOfShape(mask_shape)
    mask = op.EyeLike(tmp_tensor, k=offset)
    self_int = op.Cast(self, to=INT64)
    mask_int = op.Cast(mask, to=INT64)
...
def aten_digamma(op: OpBuilder, self: TensorType) -> TensorType:
    """digamma(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_dist(op: OpBuilder, self: TensorType, other: TensorType, p: float = 2.0) -> TensorType:
...
def aten_div(op: OpBuilder, self: TFloat, other: TFloat) -> TFloat:
...
def aten_div_complex(op: OpBuilder, self: TFloat, other: TFloat) -> TFloat:
    """div.Tensor(Tensor self, Tensor other) -> Tensor"""
...
def aten_div_mode(op: OpBuilder, self: TFloat, other: TFloat, rounding_mode: str) -> TFloat:
...
@torch_op(("aten::div.Tensor_mode", "aten::div.Scalar_mode"), trace_only=True)
def aten_div_mode_int(op: OpBuilder, self: TInt, other: TInt, rounding_mode: str) -> TInt:
    """div.Tensor_mode(Tensor self, Tensor other, *, str? rounding_mode) -> Tensor
    ...
    quotient = op.Div(op.Cast(self, to=ir.DataType.FLOAT), op.Cast(other, to=ir.DataType.FLOAT))
...
@torch_op("aten::dot")
def aten_dot(op: OpBuilder, self: TFloat, tensor: TFloat) -> TFloat:
...
def aten_dropout(op: OpBuilder, input: TFloat, p: FLOAT, train: BOOL) -> TFloat:
...
def aten_dstack(op: OpBuilder, tensors: Sequence[TensorType]) -> TensorType:
...
def aten_embedding_backward(op,
    grad: TensorType,
    indices: TensorType,
    num_weights: INT64,
    padding_idx: int,
...
def aten_embedding_bag_padding_idx(op,
    weight: TFloat,
    indices: INT64,
    offsets: INT64,
    scale_grad_by_freq: bool = False,  # pylint: disable=unused-argument
...
def aten_embedding_dense_backward(op,
    grad_output: TensorType,
    indices: TensorType,
    num_weights: INT64,
    padding_idx: int,
...
def aten_embedding_sparse_backward(op,
    grad: TensorType,
    indices: TensorType,
    num_weights: int,
    padding_idx: int,
...
def aten_empty(op: OpBuilder, size: IntType, dtype: int = FLOAT.dtype) -> TTensor:  # type: ignore[type-var]
...
def aten_empty_like(op: OpBuilder, self: TTensor, dtype: int = -1) -> TTensor:
...
def aten_eq(op: OpBuilder, self: TTensor, other: TTensor) -> BOOL:
...
@torch_op("aten::equal")
def aten_equal(op: OpBuilder, self: TTensor, other: TTensor) -> BOOL:
    ...
    elementwise_equal_int = op.Cast(elementwise_equal, to=INT64)
...
def aten_erfinv(op: OpBuilder, self: TensorType) -> TensorType:
...
@torch_op("aten::exp")
def aten_exp(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_exp2(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_expand(op: OpBuilder, self: TTensor, size: TInt) -> TTensor:
...
def aten_expand_as(op: OpBuilder, self: TTensor, other: TTensor) -> TTensor:
...
def aten_expand_copy(op: OpBuilder, self: TensorType, size: INT64, implicit: bool = False) -> TensorType:
    """expand_copy(Tensor self, SymInt[] size, *, bool implicit=False) -> Tensor"""

    raise NotImplementedError()


def aten_expm1(op: OpBuilder, self: TensorType) -> TensorType:
    """expm1(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_eye(op: OpBuilder, n: int) -> TensorType:
    """eye(int n, *, ScalarType? dtype=None, Layout? layout=None, Device? device=None, bool? pin_memory=None) -> Tensor"""

    raise NotImplementedError()


def aten_fake_quantize_per_channel_affine(op,
    self: TensorType,
    scale: TensorType,
...
def aten_fake_quantize_per_channel_affine_cachemask(op,
    self: TensorType,
    scale: TensorType,
    zero_point: TensorType,
    axis: int,
...
def aten_fake_quantize_per_channel_affine_cachemask_backward(op,
...
def aten_fake_quantize_per_tensor_affine(op,
...
def aten_fake_quantize_per_tensor_affine_cachemask(op,
    self: TensorType, scale: float, zero_point: int, quant_min: int, quant_max: int
) -> tuple[TensorType, TensorType]:
...
def aten_fake_quantize_per_tensor_affine_cachemask_backward(op,
    grad: TensorType, mask: TensorType
) -> TensorType:
...
def aten_fbgemm_linear_fp16_weight(op,
...
def aten_fbgemm_linear_fp16_weight_fp32_activation(op,
    input: TensorType, packed_weight: TensorType, bias: TensorType
) -> TensorType:
...
def aten_fbgemm_linear_int8_weight(op,
    input: TensorType,
    weight: TensorType,
    packed: TensorType,
...
def aten_fbgemm_linear_int8_weight_fp32_activation(op,
    input: TensorType,
    weight: TensorType,
    packed: TensorType,
    col_offsets: TensorType,
...
def aten_fbgemm_linear_quantize_weight(op,
    input: TensorType,
...
def aten_fbgemm_pack_gemm_matrix_fp16(op: OpBuilder, input: TensorType) -> TensorType:
    """fbgemm_pack_gemm_matrix_fp16(Tensor input) -> Tensor"""

    raise NotImplementedError()


def aten_fbgemm_pack_quantized_matrix(op: OpBuilder, input: TensorType) -> TensorType:
    """fbgemm_pack_quantized_matrix(Tensor input) -> Tensor"""

    raise NotImplementedError()


def aten_feature_alpha_dropout(op: OpBuilder, input: TensorType, p: float, train: bool) -> TensorType:
    """feature_alpha_dropout(Tensor input, float p, bool train) -> Tensor"""

    raise NotImplementedError()


def aten_feature_dropout(op: OpBuilder, input: TensorType, p: float, train: bool) -> TensorType:
...
def aten_fill(op: OpBuilder, self: TTensor, value: TTensor) -> TTensor:
...
def aten_fix(op: OpBuilder, self: TensorType) -> TensorType:
...
@torch_op("aten::flip")
def aten_flip(op: OpBuilder, self: TTensor, dims: INT64) -> TTensor:
...
def aten_fliplr(op: OpBuilder, self: TensorType) -> TensorType:
    """fliplr(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_flipud(op: OpBuilder, self: TensorType) -> TensorType:
...
@torch_op("aten::floor")
def aten_floor(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
def python_math_floor(self: TFloatOrBFloat16) -> TInt:
    """floor(Tensor self) -> Tensor"""
    floor = op.Floor(self)
    return op.Cast(floor, to=INT64)


@torch_op(("aten::floor_divide", "_operator::floordiv"))
def aten_floor_divide(op: OpBuilder, self: TFloat, other: TFloat) -> TFloat:
    """floor_divide(Tensor self, Tensor other) -> Tensor"""

    return op.Floor(op.Div(self, other))


def aten_fmax(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """fmax(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_fmin(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
@torch_op("aten::fmod")
def aten_fmod(op: OpBuilder, self: TRealOrUInt8, other: TRealOrUInt8) -> TRealOrUInt8:
...
@torch_op("aten::frac")
def aten_frac(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_frexp(op: OpBuilder, self: TensorType) -> tuple[TensorType, TensorType]:
    """frexp.Tensor(Tensor self) -> (Tensor mantissa, Tensor exponent)"""

    raise NotImplementedError()


def aten_frobenius_norm(op: OpBuilder, self: TensorType) -> TensorType:
    """frobenius_norm(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_from_file(op,
...
@torch_op("aten::full")
def aten_full(op: OpBuilder, size: INT64, fill_value: FLOAT, dtype: int = FLOAT.dtype):
...
def aten_full_like(op: OpBuilder, self, fill_value: TTensor) -> TTensor:
...
@torch_op("aten::full_like")
def aten_full_like_dtype(op: OpBuilder, self, fill_value: TTensor, dtype: int) -> TTensor:
    """full_like(Tensor self, Scalar fill_value, *, ScalarType? dtype=None, Layout? layout=None, Device? device=None, bool? pin_memory=None, MemoryFormat? memory_format=None) -> Tensor"""
...
def aten_fused_moving_avg_obs_fake_quant(op,
    self: TensorType,
...
def aten_gather(op,
    ...
    if is_scalar(index):  # When (index) is empty, return (self)
        result = self
    else:
        if is_scalar(self):  # Unsqueeze for GatherElements op
            self = op.Reshape(self, op.Constant(value_ints=[-1]))
        if op.Size(index) == 0:  # Return empty array
            result = op.CastLike(index, self)
        else:
            index = op.Cast(index, to=INT64)
            result = op.GatherElements(self, index, axis=dim)
    return result


def aten_gather_backward(op,
...
def aten_gcd(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
def aten_ge(op: OpBuilder, self: TReal, other: TReal) -> BOOL:
...
def aten_ge_bool(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
    """ge.Tensor(Tensor self, Tensor other) -> Tensor"""
...
def aten_geqrf(op: OpBuilder, self: TensorType) -> tuple[TensorType, TensorType]:
    """geqrf(Tensor self) -> (Tensor a, Tensor tau)"""

    raise NotImplementedError()


def aten_ger(op: OpBuilder, self: TensorType, vec2: TensorType) -> TensorType:
...
def aten_getitem(op: OpBuilder, self: Sequence[TTensor], i: INT64) -> TTensor:
    return op.SequenceAt(self, i)


@torch_op("aten::grid_sampler", trace_only=True)
def aten_grid_sampler(op,
    input: TTensor,
    grid: TTensor,
...
def aten_grid_sampler_2d_backward(op,
    grad_output: TensorType,
    input: TensorType,
    grid: TensorType,
    interpolation_mode: int,
...
def aten_grid_sampler_3d_backward(op,
    grad_output: TensorType,
    input: TensorType,
    grid: TensorType,
    interpolation_mode: int,
...
def aten_gt(op: OpBuilder, self: TReal, other: TReal) -> BOOL:
...
def aten_gt_bool(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
    """gt.Tensor(Tensor self, Tensor other) -> Tensor"""
    # self, other, self > other
    #    F,    F,    F
    #    F,    T,    F
...
def aten_heaviside(op: OpBuilder, self: TReal, values: TReal) -> TReal:
...
def aten_hinge_embedding_loss(op,
...
def aten_hspmm(op: OpBuilder, mat1: TensorType, mat2: TensorType) -> TensorType:
...
def aten_hstack(op: OpBuilder, tensors: Sequence[TTensor]) -> TTensor:
...
def aten_hypot(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """hypot(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_i0(op: OpBuilder, self: TensorType) -> TensorType:
    """i0(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_igamma(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """igamma(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_igammac(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """igammac(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_imag(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_index(op: OpBuilder, self: TensorType, indices: Sequence[Optional[INT64]]) -> TensorType:
...
@torch_op(("aten::index.Tensor", "aten::_unsafe_index.Tensor"), trace_only=True)
def aten_index_bool(op: OpBuilder, self: TensorType, indices: Sequence[Optional[BOOL]]) -> TensorType:  # pylint: disable=inconsistent-return-statements
    index_ranks = [len(index.shape) for index in indices if index is not None]
...
def aten_index_select(op: OpBuilder, self: TTensor, dim: int, index: IntType) -> TTensor:
...
def aten_index_select_backward(op,
...
def aten_indices(op: OpBuilder, self: TensorType) -> TensorType:
    """indices(Tensor(a) self) -> Tensor(a)"""

    raise NotImplementedError()


def aten_indices_copy(op: OpBuilder, self: TensorType) -> TensorType:
    """indices_copy(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_inner(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
def aten_int_repr(op: OpBuilder, self: TensorType) -> TensorType:
    """int_repr(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_inverse(op: OpBuilder, self: TensorType) -> TensorType:
    """inverse(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_is_coalesced(op: OpBuilder, self: TensorType) -> bool:
    """is_coalesced(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_is_complex(op: OpBuilder, self: TensorType) -> bool:
    """is_complex(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_is_conj(op: OpBuilder, self: TensorType) -> bool:
    """is_conj(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_is_distributed(op: OpBuilder, self: TensorType) -> bool:
    """is_distributed(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_is_floating_point(op: OpBuilder, self: TensorType) -> bool:
    """is_floating_point(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_is_inference(op: OpBuilder, self: TensorType) -> bool:
    """is_inference(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_is_leaf(op: OpBuilder, self: TensorType) -> bool:
    """is_leaf(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_is_neg(op: OpBuilder, self: TensorType) -> bool:
...
def aten_is_nonzero(op: OpBuilder, self: Union[RealType, BOOL]) -> BOOL:
...
def aten_is_pinned(op: OpBuilder, self: TensorType, device: Optional[str] = None) -> bool:
...
def aten_is_same_size(op: OpBuilder, self: TTensor, other: TTensor) -> BOOL:
...
def aten_is_set_to(op: OpBuilder, self: TensorType, tensor: TensorType) -> bool:
    """is_set_to(Tensor self, Tensor tensor) -> bool"""

    raise NotImplementedError()


def aten_is_signed(op: OpBuilder, self: TensorType) -> bool:
    """is_signed(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_is_vulkan_available(op: OpBuilder, ) -> bool:
...
def aten_isfinite(op: OpBuilder, self: TFloatHighPrecision) -> BOOL:
...
@torch_op("aten::isinf")
def aten_isinf(op: OpBuilder, self: TFloatOrBFloat16) -> BOOL:
...
@torch_op("aten::isnan")
def aten_isnan(op: OpBuilder, self: TFloatOrBFloat16) -> BOOL:
...
def aten_isneginf(op: OpBuilder, self: TFloatOrBFloat16) -> BOOL:
...
def aten_isposinf(op: OpBuilder, self: TFloatOrBFloat16) -> BOOL:
...
def aten_isreal(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_item(op: OpBuilder, self: TensorType) -> float:
    """item(Tensor self) -> Scalar"""

    raise NotImplementedError()


def aten_kaiser_window(op: OpBuilder, window_length: int) -> TensorType:
...
def aten_kron(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """kron(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_kthvalue(op,
...
def aten_lcm(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """lcm(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_ldexp(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
def aten_le(op: OpBuilder, self: TReal, other: TReal) -> BOOL:
...
def aten_le_bool(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
    """le.Tensor(Tensor self, Tensor other) -> Tensor"""
...
def aten_lerp(op: OpBuilder, self: TensorType, end: TensorType, weight: TensorType) -> TensorType:
    """lerp.Tensor(Tensor self, Tensor end, Tensor weight) -> Tensor"""

    raise NotImplementedError()


def aten_lgamma(op: OpBuilder, self: TensorType) -> TensorType:
    """lgamma(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_lift(op: OpBuilder, self: TensorType) -> TensorType:
    """lift(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_lift_fresh(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_lift_fresh_copy(op: OpBuilder, self: TensorType) -> TensorType:
    """lift_fresh_copy(Tensor self) -> Tensor"""

    return op.Identity(self)


def aten_linear_backward(op,
...
def aten_linspace(op,
    ...
    if steps == 0:
        return aten_full(op, op.Constant(value_ints=[0]), 0.0, dtype=dtype)
    if steps == 1:
        return aten_full(op, op.Constant(value_ints=[steps]), start, dtype=dtype)
...
@torch_op("aten::log")
def aten_log(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
@torch_op("aten::log10")
def aten_log10(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
@torch_op("aten::log1p")
def aten_log1p(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
@torch_op("aten::log2")
def aten_log2(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
def aten_logaddexp(op: OpBuilder, self: TFloatOrBFloat16, other: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
def aten_logaddexp2(op: OpBuilder, self: TFloatOrBFloat16, other: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
def aten_logcumsumexp(op: OpBuilder, self: TFloatOrBFloat16, dim: int) -> TFloatOrBFloat16:
...
def aten_logdet(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_logical_and(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
...
def aten_logical_not(op: OpBuilder, self: BOOL) -> BOOL:
...
def aten_logical_or(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
...
def aten_logical_xor(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
...
def aten_logit(op: OpBuilder, self: TFloatOrBFloat16, eps: Optional[float] = None) -> TFloatOrBFloat16:
...
def aten_logspace(op: OpBuilder, start: float, end: float, steps: int, base: float = 10.0) -> TensorType:
...
def aten_logsumexp(op: OpBuilder, self: TFloat, dim: INT64, keepdim: int = False) -> TFloat:
...
def aten_lshift(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """__lshift__.Tensor(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_lstm_cell(op,
    input: TensorType,
...
def aten_lt(op: OpBuilder, self: TReal, other: TReal) -> BOOL:
...
def aten_lt_bool(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
    """lt.Tensor(Tensor self, Tensor other) -> Tensor"""
...
def aten_lu_solve(op: OpBuilder, self: TensorType, LU_data: TensorType, LU_pivots: TensorType) -> TensorType:
    """lu_solve(Tensor self, Tensor LU_data, Tensor LU_pivots) -> Tensor"""

    raise NotImplementedError()


def aten_lu_unpack(op,
    LU_data: TensorType,
...
@torch_op("aten::mH")
def aten_mH(op: OpBuilder, self: TRealOrUInt8) -> TRealOrUInt8:
...
def aten_mH_complex(op: OpBuilder, self: TFloat) -> TFloat:
    """mH(Tensor(a) self) -> Tensor(a)"""
...
@torch_op("aten::mT")
def aten_mT(op: OpBuilder, self: TRealOrUInt8) -> TRealOrUInt8:
...
def aten_mT_complex(op: OpBuilder, self: TFloat) -> TFloat:
    """mT(Tensor(a) self) -> Tensor(a)"""
...
def aten_margin_ranking_loss(op,
    input1: TensorType,
    input2: TensorType,
    target: TensorType,
    margin: float = 0.0,
...
def aten_masked_fill(op: OpBuilder, self: TTensor, mask: BOOL, value: TTensor) -> TTensor:
...
def aten_masked_scatter(op: OpBuilder, self: TensorType, mask: TensorType, source: TensorType) -> TensorType:
    """masked_scatter(Tensor self, Tensor mask, Tensor source) -> Tensor"""

    raise NotImplementedError()


def aten_masked_select(op: OpBuilder, self: TensorType, mask: TensorType) -> TensorType:
    """masked_select(Tensor self, Tensor mask) -> Tensor"""

    raise NotImplementedError()


def aten_masked_select_backward(op,
...
def aten_matrix_H(op: OpBuilder, self: TensorType) -> TensorType:
    """matrix_H(Tensor(a) self) -> Tensor(a)"""

    raise NotImplementedError()


def aten_matrix_exp(op: OpBuilder, self: TensorType) -> TensorType:
    """matrix_exp(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_matrix_exp_backward(op: OpBuilder, self: TensorType, grad: TensorType) -> TensorType:
    """matrix_exp_backward(Tensor self, Tensor grad) -> Tensor"""

    raise NotImplementedError()


def aten_matrix_power(op: OpBuilder, self: TensorType, n: int) -> TensorType:
...
@torch_op("aten::max")
def aten_max(op: OpBuilder, self: TReal) -> TReal:
...
def aten_max_dim(op: OpBuilder, self: TReal, dim: int, keepdim: bool = False) -> Tuple[TReal, INT64]:
...
def aten_maximum(op: OpBuilder, self: TReal, other: TReal) -> TReal:
...
@torch_op(("aten::maximum", "aten::max.other"))
def aten_maximum_bool(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
    """maximum(Tensor self, Tensor other) -> Tensor"""
...
@torch_op("aten::mean")
def aten_mean(op: OpBuilder, self: TReal) -> TReal:
...
def aten_mean_dim(op: OpBuilder, self: TReal, dim: INT64, keepdim: bool = False) -> TReal:
...
def aten_median(op: OpBuilder, self: TensorType) -> TensorType:
    """median(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_meshgrid(op: OpBuilder, tensors: Sequence[TensorType]) -> TensorType:
...
@torch_op("aten::min")
def aten_min(op: OpBuilder, self: TReal) -> TReal:
...
def aten_min_dim(op: OpBuilder, self: TReal, dim: int, keepdim: bool = False) -> Tuple[TReal, TInt]:
...
def aten_minimum(op: OpBuilder, self: TReal, other: TReal) -> TReal:
...
@torch_op(("aten::minimum", "aten::min.other"))
def aten_minimum_bool(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
    """minimum(Tensor self, Tensor other) -> Tensor"""

    return op.And(self, other)


def aten_miopen_batch_norm(op,
    input: TensorType,
    weight: TensorType,
    bias: Optional[TensorType],
    running_mean: Optional[TensorType],
...
def aten_miopen_batch_norm_backward(op,
    input: TensorType,
    grad_output: TensorType,
    weight: TensorType,
    running_mean: Optional[TensorType],
...
def aten_miopen_convolution(op,
    self: TensorType,
    weight: TensorType,
    bias: Optional[TensorType],
    padding: INT64,
...
def aten_miopen_convolution_add_relu(op,
    self: TensorType,
    weight: TensorType,
    z: TensorType,
    alpha: Optional[float],
...
def aten_miopen_convolution_relu(op,
    self: TensorType,
    weight: TensorType,
    bias: Optional[TensorType],
    stride: Sequence[int],
...
def aten_miopen_convolution_transpose(op,
    self: TensorType,
    weight: TensorType,
    bias: Optional[TensorType],
    padding: INT64,
...
def aten_miopen_depthwise_convolution(op,
    self: TensorType,
    weight: TensorType,
    bias: Optional[TensorType],
    padding: INT64,
...
def aten_miopen_rnn_backward(op,
    input: TensorType,
    weight: Sequence[TensorType],
    weight_stride0: int,
...
def aten_mkldnn_adaptive_avg_pool2d(op,
...
def aten_mkldnn_adaptive_avg_pool2d_backward(op,
...
def aten_mkldnn_convolution(op,
    self: TensorType,
    weight: TensorType,
    bias: Optional[TensorType],
    padding: INT64,
...
def aten_mkldnn_linear_backward(op,
    self: TensorType, grad_output: TensorType, weight: TensorType, output_mask: Sequence[bool]
) -> tuple[TensorType, TensorType, TensorType]:
...
def aten_mkldnn_linear_backward_input(op,
...
def aten_mkldnn_linear_backward_weights(op,
...
def aten_mkldnn_max_pool2d_backward(op,
    grad_output: TensorType,
    output: TensorType,
    input: TensorType,
    kernel_size: Sequence[int],
...
def aten_mkldnn_max_pool3d_backward(op,
    grad_output: TensorType,
    output: TensorType,
    input: TensorType,
    kernel_size: Sequence[int],
...
def aten_mps_convolution_backward(op,
    self: TensorType,
    grad_output: TensorType,
    weight: TensorType,
    padding: Sequence[int],
...
def aten_mps_convolution_transpose_backward(op,
    self: TensorType,
    grad_output: TensorType,
    weight: TensorType,
    padding: Sequence[int],
...
def aten_mps_max_pool2d_backward(op,
    grad_output: TensorType,
    self: TensorType,
    kernel_size: Sequence[int],
    stride: Optional[Sequence[int]] = None,
...
def aten_msort(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_mul(op: OpBuilder, self: TReal, other: TReal) -> TReal:
...
def aten_mul_bool(op: OpBuilder, self: BOOL, other: BOOL) -> BOOL:
...
def aten_mul_complex(op: OpBuilder, self: TReal, other: TReal) -> TReal:
    """mul.Tensor(Tensor self, Tensor other) -> Tensor"""
...
def aten_multiply(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """multiply.Tensor(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_mv(op: OpBuilder, self: TensorType, vec: TensorType) -> TensorType:
    """mv(Tensor self, Tensor vec) -> Tensor"""

    raise NotImplementedError()


def aten_mvlgamma(op: OpBuilder, self: TensorType, p: int) -> TensorType:
    """mvlgamma(Tensor self, int p) -> Tensor"""

    raise NotImplementedError()


def aten_nan_to_num(op,
    self: TensorType,
...
def aten_nanmedian(op: OpBuilder, self: TensorType) -> TensorType:
    """nanmedian(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_nanquantile(op,
    self: TensorType,
    q: TensorType,
    dim: Optional[int] = None,
    keepdim: bool = False,
...
def aten_narrow(op: OpBuilder, self: TTensor, dim: INT64, start: INT64, length: INT64) -> TTensor:
...
def aten_narrow_copy(op: OpBuilder, self: TensorType, dim: int, start: INT64, length: INT64) -> TensorType:
...
def aten__native_batch_norm_no_training(op,
    input: TFloat,
    weight: Optional[TFloat] = None,
    bias: Optional[TFloat] = None,
    running_mean: Optional[TFloat] = None,
    ...
    return aten_native_batch_norm(op,
...
def aten__native_batch_norm_no_stats(op,
    input: TFloat,
    weight: Optional[TFloat] = None,
    bias: Optional[TFloat] = None,
    training: bool = False,
    ...
    return aten_native_batch_norm(op, input, weight, bias, None, None, training, momentum, eps)


@torch_op(("aten::native_batch_norm", "aten::_native_batch_norm_legit"), trace_only=True)
def aten_native_batch_norm(op,
    input: TFloat,
    weight: Optional[TFloat] = None,
    bias: Optional[TFloat] = None,
    running_mean: Optional[TFloat] = None,
...
def aten__native_batch_norm_legit_functional(op,
    input: TFloat,
    weight: Optional[TFloat] = None,
    bias: Optional[TFloat] = None,
    running_mean: Optional[TFloat] = None,
...
def aten_native_batch_norm_backward(op,
    grad_out: TensorType,
    input: TensorType,
    weight: Optional[TensorType],
    running_mean: Optional[TensorType],
...
def aten_native_channel_shuffle(op: OpBuilder, self: TensorType, groups: int) -> TensorType:
...
def aten_native_dropout_backward(op,
...
def aten_native_group_norm_backward(op,
    grad_out: TensorType,
    input: TensorType,
    mean: TensorType,
    rstd: TensorType,
...
def aten_native_layer_norm_backward(op,
    grad_out: TensorType,
    input: TensorType,
    normalized_shape: INT64,
    mean: TensorType,
...
def aten_native_norm(op: OpBuilder, self: TensorType, p: float = 2.0) -> TensorType:
...
def aten_ne(op: OpBuilder, self: TReal, other: TReal) -> BOOL:
...
def aten_neg(op: OpBuilder, self: TReal) -> TReal:
    """neg(Tensor self) -> Tensor"""

    return op.Neg(self)


def aten_negative(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_new_empty(op: OpBuilder, self: TTensor, size: INT64) -> TTensor:
...
@torch_op("aten::new_empty_strided")
def aten_new_empty_strided_dtype(op,
    self: TTensor,  # pylint: disable=unused-argument
    size: INT64,
    stride: INT64,  # pylint: disable=unused-argument
    dtype: int,
...
def aten_new_full(op: OpBuilder, self: TTensor, size: INT64, fill_value: TTensor) -> TTensor:
...
def aten_new_ones(op: OpBuilder, self: TReal, size: INT64) -> TReal:  # pylint: disable=unused-argument
...
def aten_new_zeros(op: OpBuilder, self: TReal, size: INT64) -> TReal:
...
def aten_nextafter(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
def aten_nonzero(op: OpBuilder, self: TTensor) -> INT64:
...
def aten_nonzero_numpy(op: OpBuilder, self: TensorType) -> TensorType:
    """nonzero_numpy(Tensor self) -> Tensor[]"""

    raise NotImplementedError()


def aten_norm_except_dim(op: OpBuilder, v: TensorType, pow: int = 2, dim: int = 0) -> TensorType:
...
def aten_normal_float_float(op,
...
def aten_normal_float_tensor(op: OpBuilder, mean: FLOAT, std: TFloat) -> TFloat:
...
def aten_normal_tensor_float(op: OpBuilder, mean: TFloat, std: FLOAT) -> TFloat:
...
def aten_normal_tensor_tensor(op: OpBuilder, mean: TFloat, std: TFloat) -> TFloat:
...
def aten_not_equal(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """not_equal.Tensor(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_nuclear_norm(op: OpBuilder, self: TensorType, keepdim: bool = False) -> TensorType:
...
@torch_op("aten::ones")
def aten_ones(op: OpBuilder, size: IntType, dtype: int = FLOAT.dtype):
...
def aten_ones_like(op: OpBuilder, self: TTensor, dtype: int = -1) -> TTensor:
    """ones_like.
...
def aten_or(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
    """__or__.Tensor(Tensor self, Tensor other) -> Tensor"""

    raise NotImplementedError()


def aten_orgqr(op: OpBuilder, self: TensorType, input2: TensorType) -> TensorType:
...
def aten_outer(op: OpBuilder, self: TensorType, vec2: TensorType) -> TensorType:
    """outer(Tensor self, Tensor vec2) -> Tensor"""

    raise NotImplementedError()


def aten_output_nr(op: OpBuilder, self: TensorType) -> int:
    """output_nr(Tensor self) -> int"""

    raise NotImplementedError()


def aten_pairwise_distance(op,
...
def aten_pdist(op: OpBuilder, self: TensorType, p: float = 2.0) -> TensorType:
...
def aten_permute(op: OpBuilder, self: TTensor, dims: Sequence[int]) -> TTensor:
...
def aten_permute_copy(op: OpBuilder, self: TensorType, dims: Sequence[int]) -> TensorType:
    """permute_copy(Tensor self, int[] dims) -> Tensor"""

    raise NotImplementedError()


def aten_pin_memory(op: OpBuilder, self: TensorType, device: Optional[str] = None) -> TensorType:
    """pin_memory(Tensor(a) self, Device? device=None) -> Tensor(a)"""

    raise NotImplementedError()


def aten_pinverse(op: OpBuilder, self: TensorType, rcond: float = 1e-15) -> TensorType:
    """pinverse(Tensor self, float rcond=1e-15) -> Tensor"""

    raise NotImplementedError()


def aten_pixel_shuffle(op: OpBuilder, self: TensorType, upscale_factor: int) -> TensorType:
    """pixel_shuffle(Tensor self, int upscale_factor) -> Tensor"""

    raise NotImplementedError()


def aten_pixel_unshuffle(op: OpBuilder, self: TensorType, downscale_factor: int) -> TensorType:
    """pixel_unshuffle(Tensor self, int downscale_factor) -> Tensor"""

    raise NotImplementedError()


def aten_poisson(op: OpBuilder, self: TensorType, generator: Optional[str] = None) -> TensorType:
    """poisson(Tensor self, Generator? generator=None) -> Tensor"""

    raise NotImplementedError()


def aten_poisson_nll_loss(op,
    input: TensorType,
    target: TensorType,
    log_input: bool,
    full: bool,
...
@torch_op("aten::polar")
def aten_polar(op: OpBuilder, abs: TFloat, angle: TFloat) -> TFloat:
...
def aten_polygamma(op: OpBuilder, n: int, self: TensorType) -> TensorType:
    """polygamma(int n, Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_positive(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_pow(op: OpBuilder, self: TReal, exponent: TTensor) -> TReal:
    """pow(Tensor self, Tensor exponent) -> Tensor"""

    return op.Pow(self, exponent)


def aten_prelu(op: OpBuilder, self: TensorType, weight: TensorType) -> TensorType:
    """prelu(Tensor self, Tensor weight) -> Tensor"""

    raise NotImplementedError()


def aten_prelu_backward(op,
...
def aten_prod(op: OpBuilder, self: TensorType, dtype: Optional[int] = None) -> TensorType:
    """prod(Tensor self, *, ScalarType? dtype=None) -> Tensor"""

    raise NotImplementedError()


def aten_promote_types(op: OpBuilder, type1: int, type2: int) -> int:
...
def aten_q_per_channel_axis(op: OpBuilder, self: TensorType) -> int:
    """q_per_channel_axis(Tensor self) -> int"""

    raise NotImplementedError()


def aten_q_per_channel_scales(op: OpBuilder, self: TensorType) -> TensorType:
    """q_per_channel_scales(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_q_per_channel_zero_points(op: OpBuilder, self: TensorType) -> TensorType:
    """q_per_channel_zero_points(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_q_scale(op: OpBuilder, self: TensorType) -> float:
    """q_scale(Tensor self) -> float"""

    raise NotImplementedError()


def aten_q_zero_point(op: OpBuilder, self: TensorType) -> int:
    """q_zero_point(Tensor self) -> int"""

    raise NotImplementedError()


def aten_qr(op: OpBuilder, self: TensorType, some: bool = True) -> tuple[TensorType, TensorType]:
    """qr(Tensor self, bool some=True) -> (Tensor Q, Tensor R)"""

    raise NotImplementedError()


def aten_qscheme(op: OpBuilder, self: TensorType) -> str:
    """qscheme(Tensor self) -> QScheme"""

    raise NotImplementedError()


def aten_quantile(op,
    self: TensorType,
    q: TensorType,
    dim: Optional[int] = None,
    keepdim: bool = False,
...
def aten_quantize_per_channel(op,
...
def aten_quantize_per_tensor(op,
...
def aten_quantize_per_tensor_dynamic(op,
...
def aten_quantized_batch_norm(op,
    input: TensorType,
    weight: Optional[TensorType],
    bias: Optional[TensorType],
    mean: TensorType,
...
def aten_quantized_gru_cell(op,
    input: TensorType,
    hx: TensorType,
    w_ih: TensorType,
    w_hh: TensorType,
...
def aten_quantized_lstm_cell(op,
    input: TensorType,
    hx: Sequence[TensorType],
    w_ih: TensorType,
    w_hh: TensorType,
...
def aten_quantized_max_pool1d(op,
    self: TensorType,
    kernel_size: Sequence[int],
    stride: Optional[Sequence[int]] = None,
    padding: Sequence[int] = (0,),
...
def aten_quantized_max_pool2d(op,
    self: TensorType,
    kernel_size: Sequence[int],
    stride: Optional[Sequence[int]] = None,
    padding: Sequence[int] = (0, 0),
...
def aten_quantized_rnn_relu_cell(op,
    input: TensorType,
    hx: TensorType,
    w_ih: TensorType,
    w_hh: TensorType,
...
def aten_quantized_rnn_tanh_cell(op,
    input: TensorType,
    hx: TensorType,
    w_ih: TensorType,
    w_hh: TensorType,
...
def aten_rad2deg(op: OpBuilder, self: TFloat) -> TFloat:
...
@torch_op("aten::rand")
def aten_rand(op: OpBuilder, size: INT64, dtype: int = FLOAT.dtype) -> TReal:
...
def aten_rand_like(op: OpBuilder, self: TFloat) -> TFloat:
...
@torch_op("aten::rand_like")
def aten_rand_like_dtype(op: OpBuilder, self: TensorType, dtype: int) -> TensorType:
    """rand_like(Tensor self, *, ScalarType? dtype=None, Layout? layout=None, Device? device=None, bool? pin_memory=None, MemoryFormat? memory_format=None) -> Tensor"""
...
def aten_randint(op: OpBuilder, high: INT64, size: INT64, dtype: int = INT64.dtype) -> TensorType:
...
@torch_op("aten::randint_like")
def aten_randint_like_dtype(op: OpBuilder, self: TensorType, high: INT64, dtype: int) -> TensorType:
    """randint_like(Tensor self, SymInt high, *, ScalarType? dtype=None, Layout? layout=None, Device? device=None, bool? pin_memory=None, MemoryFormat? memory_format=None) -> Tensor"""
...
def aten_randint_like_low_dtype(op: OpBuilder, self: TensorType, low: INT64, high: INT64) -> IntType:
...
@torch_op("aten::randint_like.low_dtype")
def aten_randint_like_low_dtype_dtype(op,
...
@torch_op("aten::randn")
def aten_randn(op: OpBuilder, size: INT64, dtype: int = FLOAT.dtype) -> TReal:
...
def aten_randn_like(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_reciprocal(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
    """reciprocal(Tensor self) -> Tensor"""

    return op.Reciprocal(self)


def aten_record_stream(op: OpBuilder, self: TensorType, s: str) -> Any:
    """record_stream(Tensor(a!) self, Stream s) -> ()"""

    raise NotImplementedError()


def aten_refine_names(op: OpBuilder, self: TensorType, names: Sequence[str]) -> TensorType:
...
def aten_remainder(op: OpBuilder, self: TFloatOrBFloat16, other: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
@torch_op("aten::remainder")
def aten_remainder_int(op: OpBuilder, self: TInt, other: TInt) -> TInt:
    """remainder.Tensor(Tensor self, Tensor other) -> Tensor"""

    return op.Mod(self, other)


def aten_rename(op: OpBuilder, self: TensorType, names: Optional[str]) -> TensorType:
    """rename(Tensor(a) self, Dimname[]? names) -> Tensor(a)"""

    raise NotImplementedError()


def aten_renorm(op: OpBuilder, self: TensorType, p: float, dim: int, maxnorm: float) -> TensorType:
...
def aten_repeat(op: OpBuilder, self: TTensor, repeats: TInt) -> TTensor:
    ...
    if op.Size(repeats) == 0:
        ...
        repeats = op.Cast(repeats, to=INT64)
...
def aten_reshape(op: OpBuilder, self: TTensor, shape: IntType) -> TTensor:
...
def aten_reshape_as(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
def aten_resolve_conj(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_resolve_neg(op: OpBuilder, self: TTensor) -> TTensor:
    """resolve_neg(Tensor(a) self) -> Tensor(a)"""

    return op.Identity(self)


def aten_result_type(op: OpBuilder, tensor: TensorType, other: TensorType) -> int:
    """result_type.Tensor(Tensor tensor, Tensor other) -> ScalarType"""

    raise NotImplementedError()


def aten_retain_grad(op: OpBuilder, self: TensorType) -> Any:
    """retain_grad(Tensor(a!) self) -> ()"""

    raise NotImplementedError()


def aten_retains_grad(op: OpBuilder, self: TensorType) -> bool:
    """retains_grad(Tensor self) -> bool"""

    raise NotImplementedError()


def aten_rnn_relu_cell(op,
    input: TensorType,
    hx: TensorType,
    w_ih: TensorType,
    w_hh: TensorType,
...
def aten_roll(op: OpBuilder, self: TTensor, shifts: INT64, dims: Sequence[int] = ()) -> TTensor:
...
def aten_roll_complex(op: OpBuilder, self: TTensor, shifts: INT64, dims: Sequence[int] = ()) -> TTensor:
    """roll(Tensor self, int[1] shifts, int[1] dims=[]) -> Tensor"""
...
def aten_rot90(op: OpBuilder, self: TensorType, k: int = 1, dims: Sequence[int] = (0, 1)) -> TensorType:
...
@torch_op("aten::round")
def aten_round(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_round_decimals(op: OpBuilder, self: TFloat, decimals: int = 0) -> TFloat:
...
def aten_row_indices(op: OpBuilder, self: TensorType) -> TensorType:
    """row_indices(Tensor(a) self) -> Tensor(a)"""

    raise NotImplementedError()


def aten_row_indices_copy(op: OpBuilder, self: TensorType) -> TensorType:
    """row_indices_copy(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_row_stack(op: OpBuilder, tensors: Sequence[TensorType]) -> TensorType:
...
def aten_rshift(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
@torch_op("aten::rsqrt")
def aten_rsqrt(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
def aten_rsub(op: OpBuilder, self: TReal, other: TReal, alpha: float = 1.0) -> TReal:
...
def aten_rsub_complex(op: OpBuilder, self: TReal, other: TReal, alpha: float = 1.0) -> TReal:
    """rsub.Tensor(Tensor self, Tensor other, *, Scalar alpha=1) -> Tensor"""

    return aten_rsub(op, self, other, alpha)


@torch_op("aten::scalar_tensor", trace_only=True)
def aten_scalar_tensor(op: OpBuilder, s: float, dtype: int = FLOAT.dtype) -> RealType:
...
def aten_scalar_tensor_complex(op,
...
@torch_op("aten::scalar_tensor", trace_only=True)
def aten_scalar_tensor_sym_number(op: OpBuilder, s: RealType, dtype: int = FLOAT.dtype) -> RealType:
    """scalar_tensor(Scalar s, *, ScalarType? dtype=None, Layout? layout=None, Device? device=None, bool? pin_memory=None) -> Tensor"""
...
def aten_select(op: OpBuilder, self: TTensor, dim: int, index: int) -> TTensor:
    """select(Tensor self, int dim, int index) -> Tensor"""

    return op.Gather(self, index, axis=dim)


def aten_select_backward(op,
...
def aten_select_scatter(op: OpBuilder, self: TensorType, src: TensorType, dim: int, index: int) -> TensorType:
...
@torch_op("aten::selu")
def aten_selu(op: OpBuilder, self: TFloat) -> TFloat:
    """selu(Tensor self) -> Tensor"""

    return op.Selu(self)


def aten_set_data(op: OpBuilder, self: TensorType, new_data: TensorType) -> Any:
    """set_data(Tensor(a!) self, Tensor new_data) -> ()"""

    raise NotImplementedError()


def aten_sgn(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_sigmoid(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
@torch_op("aten::sign")
def aten_sign(op: OpBuilder, self: TReal) -> TReal:
    """sign(Tensor self) -> Tensor"""

    return op.Sign(self)


def aten_signbit(op: OpBuilder, self: TensorType) -> TensorType:
...
@torch_op("aten::sin")
def aten_sin(op: OpBuilder, self: TFloat) -> TFloat:
...
@torch_op("aten::sinh")
def aten_sinh(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_slice(op,
    ...
    if start is not None:
        start = op.Cast(start, to=INT64)
        start = op.Reshape(start, op.Constant(value_ints=[-1]))
    else:
        start = op.Constant(value_ints=[0])

    if end is not None:
        end = op.Cast(end, to=INT64)
        end = op.Reshape(end, op.Constant(value_ints=[-1]))
    else:
        end = op.Constant(value_ints=[_INT64_MAX])

    dim = op.Cast(dim, to=INT64)
    dim = op.Reshape(dim, op.Constant(value_ints=[-1]))

    if step is not None:
        step = op.Cast(step, to=INT64)
...
def aten_slogdet(op: OpBuilder, self: TensorType) -> tuple[TensorType, TensorType]:
    """slogdet(Tensor self) -> (Tensor sign, Tensor logabsdet)"""

    raise NotImplementedError()


def aten_smm(op: OpBuilder, self: TensorType, mat2: TensorType) -> TensorType:
...
def aten_softmax(op: OpBuilder, self: TFloatOrBFloat16, dim: int, dtype: int = -1) -> TFloatOrBFloat16:
...
def aten_softmax_no_dtype(op: OpBuilder, self: TFloatOrBFloat16, dim: int) -> TFloatOrBFloat16:
    """softmax(Tensor self, int dim, ScalarType? dtype=None) -> Tensor"""
...
def aten_sparse_dim(op: OpBuilder, self: TensorType) -> int:
    """sparse_dim(Tensor self) -> int"""

    raise NotImplementedError()


def aten_sparse_mask(op: OpBuilder, self: TensorType, mask: TensorType) -> TensorType:
...
def aten_split(op: OpBuilder, self: TTensor, split_size: INT64, dim: int = 0) -> TTensor:
    """split.Tensor(Tensor(a -> *) self, SymInt split_size, int dim=0) -> Tensor(a)[]"""

    return op.SplitToSequence(self, split_size, axis=dim)


def aten_split_copy(op: OpBuilder, self: TensorType, split_size: INT64, dim: int = 0) -> TensorType:
...
def aten_split_with_sizes(op: OpBuilder, self: TTensor, split_sizes: INT64, dim: int = 0) -> TTensor:
    """split_with_sizes(Tensor(a -> *) self, SymInt[] split_sizes, int dim=0) -> Tensor(a)[]"""

    return op.SplitToSequence(self, split_sizes, axis=dim)


def aten_split_with_sizes_copy(op,
...
@torch_op("aten::sqrt")
def aten_sqrt(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
    """sqrt(Tensor self) -> Tensor"""

    return op.Sqrt(self)


def aten_square(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_squeeze(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_squeeze_dim(op: OpBuilder, self: TTensor, dim: int) -> TTensor:
    result = self
...
def aten_squeeze_dim_complex(op: OpBuilder, self: TTensor, dim: int) -> TTensor:
    if dim < 0:
        # Account for the complex dimension in ONNX
        dim = dim - 1

    return aten_squeeze_dim(op, self, dim)


def aten_squeeze_copy(op: OpBuilder, self: TensorType) -> TensorType:
    """squeeze_copy(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_sspaddmm(op,
...
def aten_stack_complex(op: OpBuilder, tensors: Sequence[TTensorOrString], dim: int = 0) -> TTensorOrString:
    """stack(Tensor[] tensors, int dim=0) -> Tensor"""
    # Real representation unsqueezes the last dimension
    if dim < 0:
        dim = dim - 1
    return aten_stack(op, tensors, dim)


@torch_op("aten::stack")
def aten_stack(op: OpBuilder, tensors: Sequence[TTensorOrString], dim: int = 0) -> TTensorOrString:
    """stack(Tensor[] tensors, int dim=0) -> Tensor"""
    return op.ConcatFromSequence(tensors, axis=dim, new_axis=1)


def aten_std(op: OpBuilder, self: TensorType, unbiased: bool = True) -> TensorType:
    """std(Tensor self, bool unbiased=True) -> Tensor"""

    raise NotImplementedError()


def aten_std_mean(op: OpBuilder, self: TensorType, unbiased: bool = True) -> tuple[TensorType, TensorType]:
...
def aten_sub(op: OpBuilder, self: TReal, other: TReal, alpha: float = 1.0) -> TReal:
...
def aten_sub_complex(op: OpBuilder, self: TReal, other: TReal, alpha: float = 1.0) -> TReal:
    """sub.Tensor(Tensor self, Tensor other, *, Scalar alpha=1) -> Tensor"""

    return aten_sub(op, self, other, alpha=alpha)


@torch_op(("aten::sum", "aten::sum.dim_IntList"), trace_only=True)
def aten_sum_dim_IntList(op,
...
def aten_sum_to_size(op: OpBuilder, self: TensorType, size: Sequence[int]) -> TensorType:
...
def aten_swapaxes(op: OpBuilder, self: TensorType, axis0: int, axis1: int) -> TensorType:
    """swapaxes(Tensor(a) self, int axis0, int axis1) -> Tensor(a)"""

    raise NotImplementedError()


def aten_swapdims(op: OpBuilder, self: TensorType, dim0: int, dim1: int) -> TensorType:
...
def aten_sym_size(op: OpBuilder, self: TReal, dim: int = 0) -> TReal:
...
def aten_t(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_t_copy(op: OpBuilder, self: TensorType) -> TensorType:
    """t_copy(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_take(op: OpBuilder, self: TensorType, index: TensorType) -> TensorType:
    """take(Tensor self, Tensor index) -> Tensor"""

    raise NotImplementedError()


def aten_take_along_dim(op,
...
@torch_op("aten::tan")
def aten_tan(op: OpBuilder, self: TFloat) -> TFloat:
...
@torch_op("aten::tanh")
def aten_tanh(op: OpBuilder, self: TFloat) -> TFloat:
...
def aten_tensor_bool(op: OpBuilder, self: bool, dtype: int) -> TensorType:
    tensor = op.Constant(value_int=self)
    return op.Cast(tensor, to=dtype)


@torch_op("aten::tensor.float")
def aten_tensor_float(op: OpBuilder, self: float, dtype: int) -> TensorType:
    tensor = op.Constant(value_float=self)
    return op.Cast(tensor, to=dtype)


@torch_op("aten::tensor.int")
def aten_tensor_int(op: OpBuilder, self: int, dtype: int) -> TensorType:
    tensor = op.Constant(value_int=self)
    return op.Cast(tensor, to=dtype)


def aten_tensordot(op,
...
def aten_threshold(op: OpBuilder, self: TensorType, threshold: float, value: float) -> TensorType:
    """threshold(Tensor self, Scalar threshold, Scalar value) -> Tensor"""

    raise NotImplementedError()


def aten_threshold_backward(op,
...
@torch_op("aten::tile")
def aten_tile(op: OpBuilder, self: TTensor, dims: INT64) -> TTensor:
...
def aten_to_dense(op: OpBuilder, self: TensorType, dtype: Optional[int] = None) -> TensorType:
    """to_dense(Tensor self, ScalarType? dtype=None) -> Tensor"""

    raise NotImplementedError()


def aten_to_dense_backward(op: OpBuilder, grad: TensorType, input: TensorType) -> TensorType:
    """to_dense_backward(Tensor grad, Tensor input) -> Tensor"""

    raise NotImplementedError()


def aten_to_mkldnn(op: OpBuilder, self: TensorType, dtype: Optional[int] = None) -> TensorType:
    """to_mkldnn(Tensor self, ScalarType? dtype=None) -> Tensor"""

    raise NotImplementedError()


def aten_to_mkldnn_backward(op: OpBuilder, grad: TensorType, input: TensorType) -> TensorType:
    """to_mkldnn_backward(Tensor grad, Tensor input) -> Tensor"""

    raise NotImplementedError()


def aten_to_padded_tensor(op,
...
def aten_to_sparse(op: OpBuilder, self: TensorType) -> TensorType:
    """to_sparse(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_to_sparse_bsc(op: OpBuilder, self: TensorType, blocksize: Sequence[int]) -> TensorType:
    """to_sparse_bsc(Tensor self, int[2] blocksize) -> Tensor"""

    raise NotImplementedError()


def aten_to_sparse_bsr(op: OpBuilder, self: TensorType, blocksize: Sequence[int]) -> TensorType:
    """to_sparse_bsr(Tensor self, int[2] blocksize) -> Tensor"""

    raise NotImplementedError()


def aten_to_sparse_csc(op: OpBuilder, self: TensorType) -> TensorType:
    """to_sparse_csc(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_to_sparse_csr(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_topk(op,
    ...
    if self_is_scalar:
        self = op.Unsqueeze(self, op.Constant(value_ints=[0]))
    k = op.Reshape(op.Cast(k, to=INT64), op.Constant(value_ints=[1]))
...
def aten_trace(op: OpBuilder, self: TensorType) -> TensorType:
    """trace(Tensor self) -> Tensor"""

    raise NotImplementedError()


def aten_trace_backward(op: OpBuilder, grad: TensorType, sizes: INT64) -> TensorType:
...
def aten_transpose(op: OpBuilder, self, dim0: int, dim1: int):
...
def aten_transpose_complex(op: OpBuilder, self, dim0: int, dim1: int):
    """transpose.int(Tensor(a) self, int dim0, int dim1) -> Tensor(a)"""
...
@torch_op("aten::tril")
def aten_tril(op: OpBuilder, self: TTensor, diagonal: int = 0) -> TTensor:
    """tril(Tensor self, int diagonal=0) -> Tensor"""

    return op.Trilu(self, diagonal, upper=0)


def aten_tril_indices(op: OpBuilder, row: int, col: int, offset: int = 0) -> TensorType:
...
def aten_triplet_margin_loss(op,
    anchor: TensorType,
    positive: TensorType,
    negative: TensorType,
    margin: float = 1.0,
...
@torch_op("aten::triu")
def aten_triu(op: OpBuilder, self: TTensor, diagonal: int = 0) -> TTensor:
    """triu(Tensor self, int diagonal=0) -> Tensor"""

    return op.Trilu(self, diagonal, upper=1)


def aten_triu_indices(op: OpBuilder, row: int, col: int, offset: int = 0) -> TensorType:
...
@torch_op("aten::trunc")
def aten_trunc(op: OpBuilder, self: TFloatOrBFloat16) -> TFloatOrBFloat16:
...
def aten_type_as(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
def aten_unbind(op: OpBuilder, self: TTensor, dim: int = 0) -> Sequence[TTensor]:
...
def aten_unflatten(op: OpBuilder, self: TReal, dim: INT64, sizes: INT64):
...
def aten_unfold(op: OpBuilder, self: TTensor, dimension: int, size: int, step: int) -> TTensor:
...
def aten_unfold_copy(op: OpBuilder, self: TensorType, dimension: int, size: int, step: int) -> TensorType:
    """unfold_copy(Tensor self, int dimension, int size, int step) -> Tensor"""

    raise NotImplementedError()


def aten_unique_consecutive(op,
    self: TensorType,
...
def aten_unique_dim_consecutive(op,
...
def aten_unsafe_chunk(op: OpBuilder, self: TensorType, chunks: int, dim: int = 0) -> TensorType:
    """unsafe_chunk(Tensor self, int chunks, int dim=0) -> Tensor[]"""

    raise NotImplementedError()


def aten_unsafe_split(op: OpBuilder, self: TensorType, split_size: INT64, dim: int = 0) -> TensorType:
    """unsafe_split.Tensor(Tensor self, SymInt split_size, int dim=0) -> Tensor[]"""

    raise NotImplementedError()


def aten_unsafe_split_with_sizes(op,
    self: TensorType, split_sizes: INT64, dim: int = 0
) -> TensorType:
...
def aten_unsqueeze(op: OpBuilder, self: TTensor, dim: int) -> TTensor:
...
def aten_unsqueeze_copy(op: OpBuilder, self: TensorType, dim: int) -> TensorType:
    """unsqueeze_copy(Tensor self, int dim) -> Tensor"""

    raise NotImplementedError()


def aten_value_selecting_reduction_backward(op,
...
def aten_values(op: OpBuilder, self: TensorType) -> TensorType:
    """values(Tensor(a) self) -> Tensor(a)"""

    raise NotImplementedError()


def aten_values_copy(op: OpBuilder, self: TensorType) -> TensorType:
...
def aten_var(op: OpBuilder, self: TReal, unbiased: Optional[bool] = True) -> TReal:
...
def aten_var_mean(op: OpBuilder, self: TReal, unbiased: bool = True) -> Tuple[TReal, TReal]:
...
def aten_var_mean_correction(op,
    self: TReal,
    # FIXME(justinchuby): Make dim Optional[Sequence[int]]
    dim: Optional[int] = None,
    correction: Optional[float] = None,
...
def aten_vdot(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
@torch_op("aten::view")
def aten_view(op: OpBuilder, self: TTensor, size: IntType) -> TTensor:
    """view(Tensor(a) self, SymInt[] size) -> Tensor(a)"""

    size = op.Cast(size, to=INT64)  # Reshape only support INT64 as second input
    return op.Reshape(self, size)


@torch_op("aten::view", complex=True)
def aten_view_complex(op: OpBuilder, self: TTensor, size: IntType) -> TTensor:
    """view(Tensor(a) self, SymInt[] size) -> Tensor(a)"""
...
def aten_view_as(op: OpBuilder, self: TTensor, other: TTensor2) -> TTensor:
...
def aten_view_as_complex(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_view_as_complex_copy(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_view_as_real(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_view_as_real_copy(op: OpBuilder, self: TTensor) -> TTensor:
...
def aten_view_copy(op: OpBuilder, self: TTensor, size: IntType) -> TTensor:
...
def aten_vstack(op: OpBuilder, tensors: Sequence[TTensor]) -> TTensor:
...
def aten_where(op: OpBuilder, condition: BOOL, self: TTensor, other: TTensor) -> TTensor:
    """where.self(Tensor condition, Tensor self, Tensor other) -> Tensor"""

    return op.Where(condition, self, other)


def aten_xor(op: OpBuilder, self: TensorType, other: TensorType) -> TensorType:
...
@torch_op("aten::zeros")
def aten_zeros(op: OpBuilder, size: IntType, dtype: int = FLOAT.dtype):
...
def aten_zeros_like(op: OpBuilder, self: TTensor, dtype: int = -1) -> TTensor:
